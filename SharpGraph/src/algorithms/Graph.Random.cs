...
namespace SharpGraph
{
    ...
    {
        public RandomGraphException(string msg) : base(msg) { }
    }
    partial class Graph
    {
        /// <summary>
        ...
        public static Graph GenerateRandom(int nodeCount, int edgeCount, bool isDirected, int? randSeed = null)
        {
            if (nodeCount < 1)
            {
                throw new RandomGraphException(string.Format("Cannot create graph  {0} nodes. Not enough nodes.", nodeCount));
            }
            else if (edgeCount < 0)
            {
                throw new RandomGraphException(string.Format("Cannot create graph with {0} edges. Not enough edges.", edgeCount));
            }
            else if (edgeCount > nodeCount * (nodeCount - 1) * 0.5f)
            {
                throw new RandomGraphException(string.Format("Cannot create graph with {0} edges and {1} nodes. Too many edges.", edgeCount, nodeCount));
            }
            var r = new System.Random(randSeed == null ? (int)(DateTime.Now.Ticks & 0x0000FFFF) : randSeed.Value);
            var nodes = NodeGenerator.GenerateNodes(nodeCount).ToList();
            var edges = new HashSet<(Node, Node)>();
            while (edges.Count < edgeCount)
            {
                int n1 = r.Next(nodeCount);
                int n2 = r.Next(nodeCount);
                if (n1 == n2) continue;
                if (edges.Contains((nodes[n1], nodes[n2]))
                || edges.Contains((nodes[n2], nodes[n1]))
                )
                {
                    continue;
                }
        ...
        public static Graph GenerateRandomSpanningTree(int nodeCount, int? randSeed = null)
        {
            if (nodeCount < 3)
            {
                throw new RandomGraphException(string.Format("Cannot create random spanning tree with {0} nodes. Not enough nodes.", nodeCount));
            }
            var r = new System.Random(randSeed == null ? (int)(DateTime.Now.Ticks & 0x0000FFFF) : randSeed.Value);
            var nodes = NodeGenerator.GenerateNodes(nodeCount).ToList();
