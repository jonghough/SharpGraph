...
namespace SharpGraph
{
    public partial class Graph
    {
        ...
        public bool FindMaxFlow(Node startNode, Node finishNode)
        {
            ...
            {
                var capacity = AddComponent<EdgeCapacity>(e);
                capacity.FlowDirection = Direction.Forwards;
            }

            List<Edge> allEdges = new List<Edge>(GetEdges());
            List<Edge> initialPath = FindPath(allEdges,
                startNode, finishNode, flowMemoryMap);

            while (initialPath != null)
            {
                Node last = startNode;
                Func<Edge, float> selector = e => { return GetComponent<EdgeCapacity>(e).GetResidualFlow(); };
                var minFlow = initialPath.Select(selector).Min();
                List<Edge> minNode = initialPath.FindAll(
                    e => GetComponent<EdgeCapacity>(e).GetResidualFlow() == minFlow).ToList();

                foreach (Edge e in initialPath)
                {
                    if (GetComponent<EdgeCapacity>(e).FlowDirection == Direction.Forwards)
                        GetComponent<EdgeCapacity>(e).Flow += minFlow;
                    else
                        GetComponent<EdgeCapacity>(e).Flow -= minFlow;
                ...
                foreach (Edge e in GetEdges())
                {
                    GetComponent<EdgeCapacity>(e).FlowDirection = Direction.Forwards;
                }
                ...
                initialPath.Clear();
                allEdges.Clear();
                allEdges = new List<Edge>(GetEdges());
        ...
        private List<Edge> FindPath(List<Edge> edges, Node start, Node finish, Dictionary<Node, FlowMemory> flowMemoryMap)
        {
            //filter only the edges coming from the start node
            Predicate<Edge> match = e => !edges.Contains(e);
            List<Edge> edgesList = GetIncidentEdges(start, match).Where(e =>
            {
            ...
            foreach (Edge selectedEdge in edgesList)
            {
                if (selectedEdge != null)
                {
                    ...
                    if (GetComponent<EdgeCapacity>(selectedEdge).GetResidualFlow() <= 0)
                    {
                        continue;
                    }
                    ...
                    {
                        GetComponent<EdgeCapacity>(selectedEdge).FlowDirection = Direction.Backwards;
                    }
                    ...
                    else
                    {
                        ...
                        List<Edge> subPath = FindPath(edgesCopy, next, finish, flowMemoryMap);
