...
namespace SharpGraph
{
    ...
    public partial class Graph
    {
        ...
        {
            return FindMinPathWithRouteDistances(start, finish).Item1;
        }

        public Tuple<List<Node>, Dictionary<Node, RouteMemory>> FindMinPathWithDistances(Node start, Node finish, bool isDirected = false)
        {
            return FindMinPathWithRouteDistances(start, finish);
        }

        private Tuple<List<Node>, Dictionary<Node, RouteMemory>> FindMinPathWithRouteDistances(Node start, Node finish, bool isDirected = false)
        {
            List<Node> minPath = new List<Node>();

            List<Node> nodes = new List<Node>(GetNodes());
            ...
            while (true)
            {
                ...
                Dictionary<Node, float> adjacent = GetAdjacentNodesWithWeights(min, isDirected);
