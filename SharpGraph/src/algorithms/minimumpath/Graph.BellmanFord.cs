    ...
    public partial class Graph
    {
        ...
        public List<Node> FindMinPathBF(Node nodeS, Node nodeF)
        {
            ...
            for (int i = 1; i < nodes.Count; i++)
            {
                ...
                {
                    float tmp = routeMemoryMap[edge.From()].Distance + GetComponent<EdgeWeight>(edge).Weight;
            ...
            foreach (Edge edge in GetEdges())
            {
                ...
                if (tmp < routeMemoryMap[edge.To()].Distance)
                {
                    throw new NegativeCycleException("Negative cycle found. Bellman-Ford algorithm Failure.");
                }
            }
